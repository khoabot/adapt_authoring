...
function Configuration() {
  ...
  if (process.env.dbHost) {
    this.configType = 'memory';
  } else {
    this.configType = 'file';
  }
  this.defaultConfig = path.join('conf', 'default.json');
  this.configFile = path.join('conf', 'config.json');
...
Configuration.prototype.check = function () {
  ...
  if (!(fs.existsSync || path.existsSync)(this.configFile)) {
    try {
      ...
      fs.writeFileSync(this.configFile, defaultFile);
    } catch (ex) {
      return ex.message;
    }
    return null;
...
/**
 * Loads config from environment vars or file
 *
...
Configuration.prototype.load = function (callback) {
  var config = this;
  // Check if config exists for this environment or use default
  var checkConfig = this.check();
  if (!checkConfig) {

    var Provider = nconf.Provider;
    config.nconf = new Provider();
    logger.log('info', 'config type: ' + config.configType)

    if (config.configType === 'memory') { 
        // only load specific environment variables
        config.nconf.env(['serverName', 'serverPort', 'dbType', 'dbHost', 'dbName', 'dbPort', 'dataRoot', 'tenantPrefix', 'sessionSecret', 'auth', 'useffmpeg', 'configLoaded']);
        ...
        fs.exists(this.configFile, function(exists){
          if (exists) {
            ...
            config.nconf.use('file', {file: config.configFile});
            config.nconf.load(function (err) {
                  if (err) {
                    return callback(err);
                  }
            ...
            if (config.nconf.get('configLoaded')) {
              config.emit('change:config', 'configuration');
              return callback();
            } else {
              // Currently loaded default config, install needed
              return callback();
            }

          } else {
            logger.log('info', exists);
            return callback(new MissingConfigError(config.configFile));
          }
        });
    }
    ...
    logger.log('info', 'Unable to load configuration. Error: ' + checkConfig);
    return callback(new MissingConfigError(config.configFile));
  }
...
Configuration.prototype.setConfig = function (name, value) {
  ...
  if (name && typeof name === 'string') {
    ...
    // if it is a file we need to persist to disk
    if (this.configType === 'file') {
      this.nconf.use('file', {file: this.configFile});
      this.nconf.set(name, value);
