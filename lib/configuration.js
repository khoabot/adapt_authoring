...
Configuration.prototype.load = function (filePath, callback) {
  ...
  if (config.configType === 'memory') { 
      ...
      config.nconf.env(['serverName', 'serverPort', 'dbType', 'dbHost', 'dbName', 'dbPort', 'dataRoot', 'tenantPrefix', 'sessionSecret', 'auth', 'useffmpeg'])
      ...
      fs.exists(filePath, function(exists){
        if (exists) {
          ...
          config.nconf.use('file', {file: filePath});
          ...
          logger.log('info', config.nconf.get('dbHost'));
